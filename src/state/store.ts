...
export async function setProfileFromResume(userId: string, text: string): Promise<Profile> {
  ...
  // Only what the resume yields is kept: the skills Best match reads, and the
  // length. The text itself is not stored. It was, for resume tailoring — the
  // one feature that read it back — and tailoring was removed on 25 Sep 2026.
  // Keeping a private CV nobody reads would be a liability with no use.
